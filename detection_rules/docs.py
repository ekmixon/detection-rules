...
class PackageDocument(xlsxwriter.Workbook):
    ...
    def add_rta_mapping(self):
        ...
        for row, (rule_id, mapping) in enumerate(rta_mappings.get_rta_mapping().items(), start=1):
    ...
    def add_attack_matrix(self):
        ...
        worksheet = self.add_worksheet(f'{attack_tm} Coverage')
        worksheet.freeze_panes(1, 0)
        ...
        for column, tactic in enumerate(tactics):
            ...
            for row, technique_id in enumerate(matrix[tactic], 1):
                ...
                if coverage:
                    coverage_str = '\n\n' + '\n'.join(
                        f'{sub_dir}: {count}'
                        for sub_dir, count in coverage.items()
                    )
        ...
        worksheet.autofilter(
            0, 0, max(len(v) for k, v in matrix.items()) + 1, len(tactics) - 1
        )
